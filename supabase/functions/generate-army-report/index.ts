/**
 * GTG Edge Function — generate-army-report
 *
 * U.S. Army licensing royalty compliance report (4D-2).
 ...
 * audit trail, and any active Army-authority enforcement locks.
 *
 * ─── Report structure ─────────────────────────────────────────────────────────
 *
 *   licensor      Active ARMY license_holders record — contact, rate agreement,
 ...
 *
 *   active_locks  Army-authority enforcement locks currently in force (is_active = true).
 *                 These are lock_records where lock_authority = 'army' — units that
 *                 the Army has directed GTG to hold, which must be disclosed in
 *                 the report.
 *
 * ─── Period lookup ────────────────────────────────────────────────────────────
 *
 * Accepts year_month (YYYY-MM). The royalty_entry is located by finding the
 * ARMY entry whose period_start ≤ first-day-of-month ≤ period_end. The U.S.
 * Army requires monthly reporting, so each year_month maps to exactly one entry.
 *
 * If no royalty_entry exists for the Army licensor covering the given month,
 ...
 * ─── Request ──────────────────────────────────────────────────────────────────
 *
 *   POST /functions/v1/generate-army-report
 ...
 *         "id":                        "<uuid>",
 *         "units_sold":                23,
 *         "gross_sales_cents":         114977,
 *         "royalty_rate":              0.175,
 *         "royalty_cents":             20121,
 *         "remittance_cents":          25000,
 *         "minimum_applied":           true,
 *         "status":                    "submitted",
 *         "licensor_reference_id":     "ARMY-2026-03-001",
 *         "submitted_at":              "2026-04-01T...",
 *         "submitted_by":              "<uuid>",
 *         "paid_at":                   null,
 ...
 *           "unit_id":            "<uuid>",
 *           "serial_number":      "GTG-ARMY-2026-0001",
 *           "sku":                "APP-ARMY-JERSEY-L",
 *           "product_name":       "Army Jersey — Large",
 ...
 *           "scope":                 "unit",
 *           "target_id":             "<uuid>",
 *           "target_label":          "GTG-ARMY-2026-0009",
 *           "lock_reason":           "Army IPR audit hold — duplicate serial suspected.",
 *           "licensor_reference_id": "ARMY-HOLD-2026-001",
 *           "locked_at":             "2026-03-18T..."
 *         }
 *       ]
 *     }
 *   }
 ...
 *   404  No Army royalty entry found for the given period, or no active Army licensor
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  const log = createLogger('generate-army-report', req)
  ...
  try {
    ...
    // ── Step 6: Fetch Army royalty entry for the period ─────────────────────────
    //
    // Find the royalty_entry whose period contains the given month.
    // Army uses monthly reporting, so each month maps to exactly one entry.

    authedLog.info('Fetching Army royalty entry', { year_month: yearMonth, probe_date: probeDate })

    const { data: royaltyEntry, error: royaltyError } = await admin
      .from('royalty_entries')
      .select('*')
      .eq('license_body', 'ARMY')
    ...
    if (royaltyError !== null) {
      if (royaltyError.code === 'PGRST116') {
        authedLog.warn('No Army royalty entry found', { year_month: yearMonth })
        return jsonError(
          req,
          `No Army royalty entry found for a period containing ${yearMonth}. ` +
    ...
    authedLog.info('Army royalty entry found', {
    ...
    const [licensorRes, ledgerRes, locksRes] = await Promise.all([
      ...
      // Active Army license_holders record
      admin
      ...
      // Active Army-authority enforcement locks — must be disclosed in report
      admin
        .from('lock_records')
        .select(
    ...
    if (licensorRes.error !== null) {
      if (licensorRes.error.code === 'PGRST116') {
        authedLog.warn('No active Army licensor record found')
        return jsonError(
          req,
          'No active Army license_holders record found. ' +
          'An active Army licensor record is required to generate this report.',
          404,
        )
      }
    ...
    authedLog.info('Army report assembled', {
      period_start:      entry.period_start,
      period_end:        entry.period_end,
      units_sold:        entry.units_sold,
